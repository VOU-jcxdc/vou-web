...
const roleVariants = cva("border", {
  variants: {
    role: {
      ...
      brand: "border-orange-600 bg-orange-100 text-orange-900 hover:bg-orange-100",
    },
    status: {
...
export const columns: ColumnDef<User>[] = [
  ...
  },
  {
    ...
    header: "Role",
    cell: ({ row }) => {
      const role = row.original.role as "admin" | "player" | "brand" | null | undefined;

      return (
        <Badge variant="secondary" className={cn(roleVariants({ role }), "capitalize")}>
          {role}
        </Badge>
      );
    },
  },
  {
    ...
    header: "Status",
    cell: ({ row }) => {
      const status = row.original.status as "active" | "inactive" | null | undefined;

      return (
        <Badge variant="secondary" className={cn(roleVariants({ status }), "capitalize")}>
          {status}
        </Badge>
      );
    },
